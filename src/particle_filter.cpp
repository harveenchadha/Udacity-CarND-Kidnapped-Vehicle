...
for(int i=0; i< num_particles;i++)
{
	...
	innerTerm= particles[i].theta + yawDt;
	outerTerm= velocity / yaw_rate;
	particles[i].x + = outerTerm (sin(innerTerm) - sin(particle[i]));
	particles[i].y + = outerTerm (-cos(innerTerm) + cos(particle[i]));
...
vector<LandmarkObs> translated_landmarks;
	for(int i=0;i<num_particles;i++)
	{
		...
		translated_landmarks.push_back(LandmarkObs{ observations[j].id, t_x, t_y });
...
for(int i=0;i<num_particles;i++)
{
	...
	for (int j = 0; j < map_landmarks.landmark_list.size(); j++) {
		distance_x = map_landmarks.landmark_list[j].x - particles[i].x;
		distance_y = map_landmarks.landmark_list[j].y - particles[i].y;
		if(fabs(distance_x)  <= sensor_range && fabs(distance_y) <=sensor_range)
		{
			
			predictedLandmarks.push_back(LandmarkObs{map_landmarks.landmark_list[j].id, map_landmarks.landmark_list[j].x, map_landmarks.landmark_list[j].y});
		}
	}
}
//Step 2 End
